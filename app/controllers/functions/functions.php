...
class functions
{
        ...
        foreach ($categories as $category) {
            if ($category['top']) {
                ...
                foreach ($children as $child) {
                    ...
                    $children_data[] = array(
                        'name'  => $child['name'],
                        'href'  => $this->fn_href('product/category', 'category_id=' . $child['category_id']),
                    );
                }
                ...
                $data[] = array(
                    ...
                    'href'     => $this->fn_href('product/category','category_id=' . $category['category_id'], 'C'),
                );
            }
        }
     ...
     * @return string
     */
    public function fn_url($query, $area = 'A')
    {
        $url = new Redirect();
    ...
    function fn_href($url, $query, $store_id = 0, $language_id = 1) {
        ...
        if($this->fn_settings()->get('config_seo_url')) { 
            ...
            if (!empty($seo_url)) {
                $extensionTag= '';
                if(!empty($this->fn_settings()->get('config_seo_extension'))) {
                    $extensionTag = '.'.$this->fn_settings()->get('config_seo_extension');
                }
                return $seo_url;
            }
        }
